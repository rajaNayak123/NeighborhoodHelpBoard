...
const ChatBox = ({ request, recipient }) => {
  ...
  const handleSendMessage = async (e) => {
    ...
    // Save/update the chat profile ONLY when a message is sent
    if (recipient && request) {
        const recipientProfile = { ...recipient, requestTitle: request.title };
      ...
      (savedMessage) => {
        if (savedMessage) {
            setMessages(prev => prev.map(msg =>
                msg._id === tempMessageId ? { ...savedMessage, status: 'sent' } : msg
            ));
        } else {
            setMessages(prev => prev.map(msg =>
